/**
 * functions/patch-genetics.js — Cloudflare Pages Function
 *
 * Applies genetics track structural update to Firestore using REST API
 * (no npm packages — runs natively on Cloudflare's edge runtime).
 *
 * Deploy: commit to functions/patch-genetics.js → Cloudflare Pages auto-deploys.
 * Invoke: https://<your-domain>/patch-genetics?secret=<SEED_SECRET>
 * Delete after use.
 *
 * Reads from env:
 *   FIREBASE_SERVICE_ACCOUNT  — same JSON string used by the scanner worker
 *   SEED_SECRET               — guards the endpoint
 */
...
async function fsCreate(docPath, data, token) {
  const fields = Object.fromEntries(Object.entries(data).map(([k,v]) => [k, toV(v)]));
  ...
  return {
    ...
    'g-4.5': {
      ...
      milestones: [
        ...
        { id: 'g-4.5.2', completed: false, evidence: '', name: 'Murine in vivo model: engineered cells confirm triage activation within 24h, fibrotic capsule at ≥85% of sites, vascular peptide ≥10 fmol/mL by systemic assay; host biological age unchanged at 90-day follow-up (no SASP-mediated bystander aging).' },
        { id: 'g-4.5.3', completed: false, evidence: '', name: 'Large animal / NHP validation across ≥3 tissue compartments: independent triage activation in each, no cross-compartment corruption propagation, vascular peptide ≥10 fmol/mL by clinical-grade ELISA, host biological age within ±5% of baseline at 180-day follow-up.' },
        { id: 'g-4.5.4', completed: false, evidence: '', name: 'Human Phase I safety and biomarker detection trial (n ≥ 12): vascular peptide ≥10 fmol/mL in serum by LC-MS/MS within 72h; no grade ≥2 adverse events (CTCAE v5.0); CRP ≤1.5× ULN at 30 days; no fibrotic lesion expansion beyond 5mm by MRI.' }
      ],
      anchors: ['Byzantine fault tolerance — Lamport, Shostak, Pease (1982)', 'Synthetic gene circuit design — Collins Lab MIT / iGEM', 'Sirius Red collagen staining — standard fibrosis quantification assay', 'Bio-orthogonal chemistry — Bertozzi Lab Stanford', 'LC-MS/MS peptide quantification — clinical proteomics gold standard', 'CTCAE v5.0 — Common Terminology Criteria for Adverse Events (FDA/NCI)'],
      status: 'active', lastUpdated: now, milestonesUpdatedAt: now
    },
    'g-4.6': {
      id: 'g-4.6', track: 'genetics', phase: 4, order: 15,
      phaseLabel: 'Phase 4 — Intercellular Communication Layer',
      name: 'Full Chromosomal Integration and Identity Verification',
      shortName: 'Full Chromosomal Integration', isBoss: false,
      theoreticalTarget: 'Demonstrate simultaneous, non-interfering co-expression of all 14 Track 01 subsystems — epigenetic reprogramming (G-1.1), senolytic clearance (G-1.2), vascular gene delivery (G-1.3), telomere maintenance (G-1.4), heterochromatin decoupling (G-2.1), precision editing (G-2.2), nuclear enclaves (G-3.1), cryptographic logic gates (G-3.2), identity tag binding (G-3.3), tunneling nanotube synthesis (G-4.1), P2P re-flashing (G-4.2), stem cell routing (G-4.3), somatic identity detection (G-4.4), and somatic identity triage (G-4.5) — within a single aged mammalian organism. Confirm systemic somatic identity integrity (synthetic identity tag readable in ≥85% of sampled cells across ≥5 tissue types), measurable biological age reduction of ≥20% by two independent epigenetic clocks, and 12-month safety with zero oncogenic events, stable karyotype by whole-genome sequencing, and ANA panel negative throughout.',
      milestones: [
        { id: 'g-4.6.1', completed: false, evidence: '', name: 'All 14 Track 01 subsystems co-delivered and co-expressed in a single aged murine model without mutual interference — confirmed by multi-omics profiling.' },
        { id: 'g-4.6.2', completed: false, evidence: '', name: 'Systemic somatic identity verification: identity tag readable in ≥85% of sampled cells across ≥5 tissue types at 6-month timepoint.' },
        { id: 'g-4.6.3', completed: false, evidence: '', name: 'Measurable multi-system functional rejuvenation — biological age reduction ≥20% by two independent epigenetic clocks (Horvath + GrimAge/DunedinPACE) in same organism.' },
        { id: 'g-4.6.4', completed: false, evidence: '', name: 'Safety at 12-month follow-up: zero oncogenic events, ANA panel negative, karyotype stable by whole-genome sequencing.' }
      ],
      anchors: ['Horvath epigenetic clock — whole-organism aging biomarker', 'Multi-omics integration — Snyder Lab Stanford', 'ANA (anti-nuclear antibody) panel — standard autoimmunity safety screen', 'Whole-genome sequencing for karyotype stability — Illumina / PacBio long-read', 'Biological age reduction in vivo — Sinclair Lab OSK eye rejuvenation (Nature 2020)', 'AAVS1 safe harbour locus — Porteus Lab Stanford'],
      status: 'active', lastUpdated: now, milestonesUpdatedAt: now
    }
  };
}
