...
The Radical Longevity Index is maintained as a live, continuously updated research tracking infrastructure rather than a static publication. The currency of milestone completion data is sustained by an automated AI literature scanning pipeline executed on a six-hour cycle via Cloudflare Workers, a distributed edge-computing infrastructure operating across 300+ globally distributed nodes providing sub-50ms execution latency without centralised server dependencies.

Each pipeline cycle executes the following stages. Stage A loads all obstacle and milestone records from the Firestore database and identifies milestones not yet marked complete. Stage B constructs a structured evaluation prompt for each obstacle, listing its name and all unconfirmed milestone descriptions, and submits this prompt to the Google Gemini 2.5 Flash large language model via the Generative Language API. Stage C parses the structured JSON response returned by the model. Stage D writes confirmed milestones — those returned with achieved=true and confidence ∈ {high, medium} — to the Firestore obstacle database, recording the evidence citation and confirmation timestamp. Stage E pushes the updated obstacle state to the public GitHub repository and dispatches email notifications to subscribers.

Milestone evaluation is performed by the language model based on its training knowledge as of its knowledge cutoff date. This design prioritises operational simplicity and zero external API dependencies, with the acknowledged limitation that new publications appearing after the model's training cutoff are not incorporated until the model is updated.

### 4.2 Evaluation Criteria

The evaluation prompt instructs the model to assess each milestone against the following criterion: a milestone is ACHIEVED if there exists direct experimental evidence meeting the specific quantitative threshold described in the milestone definition, published in a reputable peer-reviewed journal. A milestone is NOT YET ACHIEVED if the available evidence is theoretical, limited to preprints, derived from incomplete clinical trials, or does not meet the quantitative threshold. The prompt explicitly excludes partial evidence, computational predictions, and pilot studies as qualifying evidence.

The model is instructed to apply conservative judgement: when evidence is ambiguous or the model's confidence is low, the NOT YET ACHIEVED verdict is required regardless of suggestive positive findings. This asymmetric error preference is designed to minimise false-positive milestone completions at the cost of some delay in recognising genuine confirmations.

These criteria are expressed as prompt instructions to the language model rather than as algorithmically enforced hard gates. Accordingly, their consistent application depends on the model's interpretation and training. All evidence citations generated by the pipeline are model-produced and should be independently verified against the primary literature before being treated as authoritative. The public GitHub repository provides a structured mechanism for the scientific community to submit milestone challenges via pull requests.

### 4.3 LLM Evaluation Structure

The evaluation uses a single structured prompt submitted per obstacle. The prompt provides the obstacle name and a numbered list of unconfirmed milestone descriptions. For each milestone the model returns a JSON object containing three fields: achieved (boolean), confidence (one of: high, medium, low), and evidence (a single sentence citing the specific paper, authors, journal, and year, or null if not achieved).

Milestones are committed if and only if achieved is true and confidence is high or medium. Low-confidence results are not committed regardless of the achieved verdict, in order to suppress speculative confirmations. The evidence field is stored alongside the milestone record and displayed publicly on the dashboard and in subscriber notifications.

The full prompt and response for each obstacle are not currently archived in the public repository. The GitHub commit history provides a public, append-only record of all milestone state transitions, with each commit timestamped and attributable to the automated pipeline. The repository's pull-request mechanism is the designated channel for milestone challenges from the scientific community.
